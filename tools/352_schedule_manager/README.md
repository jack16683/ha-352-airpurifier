...
可选择扫描或手动输入 IP。选定设备后会先自动查询并显示 4 个定时槽，
无需再手动选择 `1. 查询定时`；菜单中的查询选项仍可用于刷新。
...
定义的只读 `0x23` 定向发现包。Windows 使用 `arp -a`，Linux 使用
`/proc/net/arp`、`ip neigh` 和 `arp`；macOS 会在邻居表没有候选时执行
受控并发 ICMP 探测，再通过 `arp -n -a` 读取结果。发现响应会提供实际
协议族、company 和设备鉴权码。

扫描会执行两轮邻居表刷新与主动发现。如果系统不允许 Python 读取邻居表，
主动发现失败后，工具会监听最长 45 秒（覆盖至少两个常见广播周期），从
净化器的周期状态广播直接学习 IP、MAC、协议族和鉴权码。因此自动扫描和
仅填写 IP 不会只依赖某一个操作系统命令；主动发现成功时仍会提前结束等待。
